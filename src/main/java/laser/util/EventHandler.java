...
public class EventHandler {
	...
	public void onLivingDrop(LivingDropsEvent event) {
		if (event.entityLiving instanceof EntityGhast && event.recentlyHit) {
			...
			entityitem.setEntityItemStack(ItemHelper.cloneStack(Laser.tentacleGhast,
					event.entityLiving.worldObj.rand.nextInt(3 + event.lootingLevel)));
	...
	public void onInteractEntity(EntityInteractEvent event) {
		...
		if (event.target instanceof EntityVillager && itemstack != null && itemstack.getItem() == Items.bucket
