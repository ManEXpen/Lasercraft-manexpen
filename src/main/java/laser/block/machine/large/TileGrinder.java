...
public class TileGrinder extends TileLargeMachineBase {
         ...
         for(int i = 9; !paramArrayList.isEmpty() && i < 18; ++i) {
            while(!paramArrayList.isEmpty()) {
               if(this.inventory[i] == null) {
                  ...
                  if(this.inventory[i].stackSize + localItemStack1.stackSize > localItemStack1.getMaxStackSize()) {
   ...
   protected void processFinish() {
      this.worldObj.playSoundEffect((double)this.xCoord + 0.5D, (double)this.yCoord + 0.5D, (double)this.zCoord + 0.5D, "mob.irongolem.death", 0.5F, this.worldObj.rand.nextFloat() / 10.0F + 0.7F);
      GrinderManager.RecipeGrinder[] localRecipesGrinder = new GrinderManager.RecipeGrinder[9];
      ArrayList localArrayList = new ArrayList();
   ...
   public ArrayList getCasingCoords() {
      ...
      for(int i = -1; i <= 1; ++i) {
         for(int j = -1; j <= 1; ++j) {
            for(int k = -1; k <= 1; ++k) {
               if(this.worldObj.getBlock(centerX + i, centerY + j, centerZ + k) == Laser.casing && this.worldObj.getBlockMetadata(centerX + i, centerY + j, centerZ + k) == Laser.casingSteel.getItemDamage()) {
                  coords.add(new Integer[]{Integer.valueOf(centerX + i), Integer.valueOf(centerY + j), Integer.valueOf(centerZ + k)});
               }
            }
         }
      }
   ...
   public boolean isComplete() {
      ForgeDirection face = ForgeDirection.getOrientation(this.getFacing());
      return !this.worldObj.isAirBlock(this.xCoord - face.offsetX, this.yCoord, this.zCoord - face.offsetZ)?false:this.getCasingCoords().size() == 25;
   }
