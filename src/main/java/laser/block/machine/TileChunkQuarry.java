...
public class TileChunkQuarry extends TileMachineBase {
   ...
   public void updateEntity() {
      ...
      if(ServerHelper.isClientWorld(this.worldObj) && this.showBorder && this.timeCheckEighth() && !this.isContainItself()) {
      ...
      if(this.isActive) {
         ...
         for(i = 0; i < this.entityQuarryLaser1.length; ++i) {
            if(this.entityQuarryLaser1[i] == null) {
               this.entityQuarryLaser1[i] = new EntityQuarryLaser(this.worldObj, this, ForgeDirection.getOrientation(i + 2));
   ...
   protected boolean canStart() {
      Chunk chunk = this.worldObj.getChunkFromChunkCoords(this.chunkCoordX, this.chunkCoordZ);
      return this.aimY >= 0 && this.aimY < 256 && !(chunk instanceof EmptyChunk) && !this.isContainItself() && this.energyStorage.getEnergyStored() >= this.getRequireEnergy();
   }
   ...
   private int getRequireEnergy() {
      if(this.isContainItself()) {
         return 0;
      } else if(this.aimY >= 0 && this.aimY < 256 && !(this.worldObj.getChunkFromChunkCoords(this.chunkCoordX, this.chunkCoordZ) instanceof EmptyChunk)) {
         ...
         for(int i = beginX; i < endX; ++i) {
            for(int j = beginZ; j < endZ; ++j) {
               if(i == beginX || i == endX - 1 || j == beginZ || j == endZ - 1) {
                  ...
                  if(block instanceof IFluidBlock || block == Blocks.water || block == Blocks.flowing_water || block == Blocks.lava || block == Blocks.flowing_lava) {
   ...
   protected void processFinish() {
      ...
      for(int i = beginX; i < endX; ++i) {
         for(int j = beginZ; j < endZ; ++j) {
            Block block = this.worldObj.getBlock(i, this.aimY, j);
            if(!block.isAir(this.worldObj, i, this.aimY, j) && block.getBlockHardness(this.worldObj, i, this.aimY, j) >= 0.0F) {
               this.stacks.addAll(block.getDrops(this.worldObj, i, this.aimY, j, this.worldObj.getBlockMetadata(i, this.aimY, j), 0));
