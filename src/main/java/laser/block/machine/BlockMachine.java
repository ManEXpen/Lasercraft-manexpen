...
public class BlockMachine extends BlockTEBase {
	...
	public void breakBlock(World paramWorld, int paramInt1, int paramInt2, int paramInt3, Block paramBlock,
			int paramInt4) {
		if (paramInt4 == 1) {
			TileChunkQuarry tilechunkquarry = (TileChunkQuarry) paramWorld.getTileEntity(paramInt1, paramInt2,
					paramInt3);
	...
	public static ItemStack setStackTag(ItemStack paramItemStack) {
		if (paramItemStack.getItem() instanceof ItemBlockMachine) {
			if (paramItemStack.getItemDamage() == 1) {
				...
				ReconfigurableHelper.setSideCache(paramItemStack,
						TileMachineBase.defaultSideConfig[paramItemStack.getItemDamage()].defaultSides);
	...
	public void onBlockPlacedBy(World paramWorld, int paramInt1, int paramInt2, int paramInt3,
		...
		if (paramItemStack.stackTagCompound != null) {
			TileMachineBase localTileMachineBase = (TileMachineBase) paramWorld.getTileEntity(paramInt1, paramInt2,
					paramInt3);
			...
			byte[] arrayOfByte = ReconfigurableHelper.getSideCache(paramItemStack,
					localTileMachineBase.getDefaultSides());
			...
			localTileMachineBase.sideCache[BlockHelper.getOppositeSide(i)] = arrayOfByte[BlockHelper
					.getOppositeSide(j)];
			if (localTileMachineBase instanceof TileChunkQuarry) {
				System.out.println("Chunk Quarry Placed!");
				...
				((TileChunkQuarry) localTileMachineBase).buildBarrier = ChunkQuarryHelper
						.isBuildBarrier(paramItemStack);
			}
	...
	public boolean onBlockActivated(World paramWorld, int paramInt1, int paramInt2, int paramInt3,
		...
		System.out.println("Called onBlockActivated");
		...
		if (localTileEntity instanceof TileChunkQuarry) {
			...
			if (localItemStack != null && localItemStack.getItem() instanceof ItemChunkUtil
					&& localItemStack.getItemDamage() == 1 && localItemStack.stackTagCompound != null
					&& localItemStack.stackTagCompound.hasKey("Chunk")) {
		...
		return super.onBlockActivated(paramWorld, paramInt1, paramInt2, paramInt3, paramEntityPlayer, paramInt4,
	...
	public IIcon getIcon(int paramInt1, int paramInt2) {
		return paramInt1 == 0 ? IconRegistry.getIcon("LCMachineBottom")
				: (paramInt1 == 1 ? IconRegistry.getIcon("LCMachineTop")
						: (paramInt1 != 3 ? IconRegistry.getIcon("LCMachineSide")
								: IconRegistry.getIcon("LCMachineFace" + paramInt2)));
	}
	...
	public void registerBlockIcons(IIconRegister paramIIconRegister) {
		for (int i = 0; i < TYPES.length; ++i) {
			...
			IconRegistry.addIcon("LCMachineFace" + i, "laser:machine/Machine" + StringHelper.titleCase(TYPES[i]),
					paramIIconRegister);
			IconRegistry.addIcon("LCMachineActive" + i,
					"laser:machine/Machine" + StringHelper.titleCase(TYPES[i]) + "_Active", paramIIconRegister);
		}
