...
public class BlockFrame extends Block implements IDismantleable {
	...
	public boolean canCreatureSpawn(EnumCreatureType paramEnumCreatureType, IBlockAccess paramIBlockAccess,
	...
	public ArrayList dismantleBlock(EntityPlayer paramEntityPlayer, World paramWorld, int paramInt1, int paramInt2,
		...
		if (!paramBoolean) {
			...
			double d1 = (double) (paramWorld.rand.nextFloat() * localArrayList)
					+ (double) (1.0F - localArrayList) * 0.5D;
			double d2 = (double) (paramWorld.rand.nextFloat() * localArrayList)
					+ (double) (1.0F - localArrayList) * 0.5D;
			double d3 = (double) (paramWorld.rand.nextFloat() * localArrayList)
					+ (double) (1.0F - localArrayList) * 0.5D;
			EntityItem localEntityItem = new EntityItem(paramWorld, (double) paramInt1 + d1, (double) paramInt2 + d2,
					(double) paramInt3 + d3, localItemStack);
			localEntityItem.delayBeforeCanPickup = 10;
			paramWorld.spawnEntityInWorld(localEntityItem);
			CoreUtils.dismantleLog(paramEntityPlayer.getCommandSenderName(), this, i, (double) paramInt1,
					(double) paramInt2, (double) paramInt3);
		}
	...
	public boolean canDismantle(EntityPlayer paramEntityPlayer, World paramWorld, int paramInt1, int paramInt2,
			int paramInt3) {
		return true;
	}
